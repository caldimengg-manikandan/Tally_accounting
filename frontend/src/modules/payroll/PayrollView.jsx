    ...
    <div className="p-8 max-w-[1400px] mx-auto space-y-8 animate-fade-in">
      ...
      <div className="flex justify-between items-end border-b border-slate-100 pb-8">
        <div>
          ...
          <h1 className="text-3xl font-bold text-slate-900 tracking-tighter">Payroll Management</h1>
        </div>
        <button onClick={() => { setShowModal(true); setError(''); setSuccess(''); }}
          className="bg-violet-600 text-white px-6 py-3 rounded-xl font-bold text-xs uppercase tracking-widest shadow-xl flex items-center gap-2 hover:bg-violet-700 transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div className="bg-violet-50 rounded-[2rem] border border-violet-100 p-7 flex justify-between items-center">
          <div>
            <p className="text-[10px] font-bold text-violet-400 uppercase tracking-widest mb-1">Total Payroll Paid</p>
            <h3 className="text-2xl font-bold text-violet-700 tracking-tighter">{fmt(totalPaid)}</h3>
          </div>
        ...
        <div className="bg-white rounded-[2rem] border border-slate-100 p-7 flex justify-between items-center shadow-sm">
          <div>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Payment Vouchers</p>
            <h3 className="text-2xl font-bold text-slate-900 tracking-tighter">{payrollHistory.length}</h3>
          </div>
        </div>
        <div className="bg-white rounded-[2rem] border border-slate-100 p-7 flex justify-between items-center shadow-sm">
          <div>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">This Month</p>
            <h3 className="text-2xl font-bold text-slate-900 tracking-tighter">{MONTHS[now.getMonth()]} {now.getFullYear()}</h3>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[2rem] border border-slate-100 shadow-xl overflow-hidden">
        <div className="h-14 px-8 bg-slate-50/50 border-b border-slate-100 flex items-center justify-between">
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Payment History</span>
        </div>
            ...
            {payrollHistory.length === 0 ? (
              ...
              return (
                <tr key={v.id} className="hover:bg-violet-50/20 transition-all">
                  <td className="px-8 py-4 font-bold text-violet-600">{v.voucherNumber || '—'}</td>
                  ...
                  <td className="px-8 py-4 text-right font-bold">{fmt(amount)}</td>
                </tr>
              );
            })}
          </tbody>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-lg overflow-hidden">
            <div className="px-8 py-6 border-b border-slate-100 flex justify-between items-center">
              <div>
                <h2 className="text-lg font-bold text-slate-900">Post Salary Voucher</h2>
            ...
            <div className="p-8 space-y-5">
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Employee / Salary Expense Ledger *</label>
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Salary Amount (₹) *</label>
                <input type="number" value={form.amount} onChange={e => setForm(f => ({ ...f, amount: e.target.value }))}
                  placeholder="0.00"
                  className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-700 outline-none focus:border-violet-400 transition-all" />
              </div>
              <div>
                <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Narration (optional)</label>
            ...
            <div className="px-8 py-5 border-t border-slate-100 flex justify-end gap-3">
              ...
              <button onClick={handlePost} disabled={saving}
                className="px-6 py-2.5 rounded-xl bg-violet-600 text-white text-sm font-bold hover:bg-violet-700 transition-all flex items-center gap-2 disabled:opacity-50">
