...
import { 
  Plus, X, ChevronDown, 
  Info, Target, Layout,
  Settings, Check, Search, Trash2, ArrowLeft, TrendingUp, AlertCircle, BarChart3, HelpCircle
} from 'lucide-react';
import { budgetAPI, ledgerAPI } from '../../services/api';
...
export default function BudgetsView({ showNew }) {
  ...
  const addItem = () => {
    if (!selectedLedgerId) return alert('Please select a ledger');
    if (!targetAmount || parseFloat(targetAmount) <= 0) return alert('Please enter a valid target amount');

    // Check duplicate
    if (formData.items.some(item => item.ledgerId === selectedLedgerId)) {
      return alert('Ledger is already added to this budget');
    }
  ...
  const removeItem = (ledgerId) => {
    setFormData({
      ...formData,
      items: formData.items.filter(item => item.ledgerId !== ledgerId)
    });
  };

  const filteredLedgers = ledgers.filter(l => 
    l.name.toLowerCase().includes(searchLedger.toLowerCase())
  );
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 bg-white rounded-3xl border border-slate-100 shadow-xl space-y-6 max-w-3xl mx-auto p-10">
              ...
              <p className="text-slate-500 text-xs leading-relaxed text-center px-10">
                Establish corporate budgeting structures, define spending limits for distinct accounts, and view automated variance dashboards mapping real-time double-entry ledgers.
              </p>
              <button 
                onClick={() => setActiveView('create')}
            ...
            <form onSubmit={handleCreateBudget} className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="flex flex-col gap-1.5">
                  ...
                  <input 
                    ...
                    placeholder="e.g. FY27 Travel & Marketing Budget"
              ...
              <div className="border-t border-slate-100 pt-6 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end bg-slate-50 p-5 rounded-2xl border border-slate-100">
                  <div className="md:col-span-6 flex flex-col gap-1.5">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Select Ledger Account</label>
                    <select 
                  ...
                  <div className="md:col-span-4 flex flex-col gap-1.5">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Target Cap Amount (₹)</label>
                    <input 
                      type="number" 
                      value={targetAmount}
                ...
                <div className="border border-slate-100 rounded-2xl overflow-hidden bg-white">
                  <table className="w-full text-left">
                    <thead className="text-[10px] font-bold uppercase tracking-[0.1em] text-slate-400 border-b border-slate-100 bg-slate-50/50">
                      <tr>
                        <th className="px-6 py-3.5">Ledger Account</th>
                      ...
                      {formData.items.length > 0 ? (
                        formData.items.map((item, idx) => (
                          <tr key={idx} className="hover:bg-slate-50/40">
                            <td className="px-6 py-3.5">{item.ledgerName}</td>
                            <td className="px-6 py-3.5 text-right">{fmt(item.targetAmount)}</td>
                            <td className="px-6 py-3.5 text-center">
                              <button 
                                type="button" 
                                onClick={() => removeItem(item.ledgerId)}
                        ...
                        <tr>
                          <td colSpan={3} className="text-center py-8 text-slate-400 text-xs">
                            No ledger accounts budgeted. Select an account above to add.
                          </td>
                        </tr>
                      )}
                    </tbody>
              ...
              <div className="flex gap-3 justify-end pt-4 border-t border-slate-100">
                ...
                <button 
                  type="submit"
                  disabled={loading}
                  className="px-8 py-3 bg-slate-900 hover:bg-slate-800 text-white rounded-xl font-bold text-xs uppercase tracking-widest shadow-xl shadow-slate-900/10 transition-all disabled:opacity-50"
                >
                    ...
                    return (
                      <div key={idx} className="space-y-2">
                        <div className="flex justify-between items-end text-xs">
                          <div>
                            <span className="font-extrabold text-slate-800">{item.ledgerName}</span>
                          ...
                          <div className="text-right">
                            ...
                            <span className={`ml-2 font-black ${pct > 90 ? 'text-red-600' : 'text-slate-500'}`}>
                              ({pct.toFixed(1)}%)
                            </span>
                          </div>
                        </div>
                ...
                <table className="w-full text-left">
                  <thead className="text-[10px] font-bold uppercase tracking-[0.15em] text-slate-400 border-b border-slate-50 bg-[#fcfdfe]">
                    <tr>
                      ...
                      <th className="px-8 py-4 text-right">Target Limit (₹)</th>
                      <th className="px-8 py-4 text-right">Actual Transaction (₹)</th>
                      <th className="px-8 py-4 text-right">Variance Balance (₹)</th>
                      <th className="px-8 py-4 text-center">Filing Achievement (%)</th>
                    </tr>
                  </thead>
                    ...
                    {varianceData.items?.length > 0 ? (
                      varianceData.items.map((item, idx) => (
                        <tr key={idx} className="hover:bg-slate-50/50">
                          <td className="px-8 py-4 font-bold text-slate-900">{item.ledgerName}</td>
