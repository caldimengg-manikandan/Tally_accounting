...
import { voucherAPI, ledgerAPI, purchaseAPI, inventoryAPI } from '../../services/api';
...
const ExpenseEntryView = ({ companyId }) => {
  ...
  const handleSave = async (isNew = false) => {
    ...
    if (isItemized) {
       ...
       if (validRows.length === 0) {
          setError("Please add at least one valid expense item with an account and amount.");
          return;
       }
       validRows.forEach(r => {
       ...
       if (!formData.expenseAccountId || !formData.amount) {
          setError("Please fill out required fields: Expense Account and Amount.");
          return;
       }
    ...
    try {
      ...
      console.error(err);
      setError("Failed to save expense. " + (err.response?.data?.error || err.message));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="bg-white min-h-screen flex flex-col font-sans">
      ...
      <div className="flex-1 max-w-[1100px] w-full mx-auto p-8 grid grid-cols-[1fr_350px] gap-12">
         ...
         <div className="space-y-6">
            ...
            {/* Top section changes depending on single/itemized */}
            {!isItemized ? (
               <>
