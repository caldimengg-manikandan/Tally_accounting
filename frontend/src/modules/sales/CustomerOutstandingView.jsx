  ...
  return (
    <div className="p-8 max-w-[1600px] mx-auto space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-4 gap-5">
        {[
          { label: 'Total Invoiced',   value: fmt(totals.total),       color: 'bg-blue-50/50 text-blue-900 border border-blue-100/70',           icon: TrendingUp },
          { label: 'Total Received',   value: fmt(totals.received),    color: 'bg-emerald-50/50 text-emerald-900 border border-emerald-100/70', icon: CheckCircle2 },
          { label: 'Outstanding',      value: fmt(totals.outstanding), color: 'bg-amber-50/50 text-amber-900 border border-amber-100/70',       icon: AlertCircle },
          { label: 'Overdue',          value: fmt(filtered.filter(i => i.status === 'Overdue').reduce((s, i) => s + i.outstanding, 0)),
                                              color: 'bg-rose-50/50 text-rose-900 border border-rose-100/70',             icon: Clock },
        ].map(card => {
