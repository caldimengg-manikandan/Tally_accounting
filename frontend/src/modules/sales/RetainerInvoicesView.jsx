...
import { 
  ...
  ChevronDown, Filter, Loader2, Clock, Mail, Printer, MoreVertical, X, Save, Send, ArrowLeft, ExternalLink, MoreHorizontal, Settings, RefreshCw, RotateCcw, Edit
} from 'lucide-react';
...
const RetainerInvoiceForm = ({ companyId, navigate, editId }) => {
    return (
        <div className="flex-1 flex flex-col items-center justify-center p-20 bg-white min-h-[70vh]">
            <div className="w-16 h-16 bg-blue-50 rounded-full flex items-center justify-center text-blue-600 mb-6">
                <FileText size={32} />
            </div>
            <h2 className="text-xl font-bold text-slate-900 mb-2">Retainer Creation</h2>
            <p className="max-w-md text-center text-slate-500 text-[14px] mb-8">
                The creation engine is being synced with the standard module architecture.
            </p>
            <button onClick={() => navigate('/retainer-invoices')} className="px-6 py-2 bg-slate-900 text-white rounded-md font-medium text-[13px] hover:bg-slate-800 transition-all shadow-sm">
                Back to List
            </button>
        </div>
    );
                ...
                <div id="printable-retainer" className="bg-white w-full max-w-[800px] shadow-sm border border-slate-200 min-h-[1000px] p-16 print:shadow-none print:border-0 rounded-sm">
                    ...
                    <table className="w-full text-left border-collapse mb-10">
                        ...
                        <tbody className="divide-y divide-slate-50 text-[14px]">
                            <tr>
                                <td className="px-4 py-6 font-medium">Retainer Payment Advance</td>
                                <td className="px-4 py-6 text-right font-bold text-slate-900">₹{parseFloat(retainer.totalAmount || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                            </tr>
                        </tbody>
                    </table>
    ...
    <div className="bg-white min-h-screen no-print">
      ...
      <div className="flex items-center justify-between px-8 py-4 border-b border-slate-100">
          <div className="flex items-center gap-2 group cursor-pointer">
             <h1 className="text-[20px] font-bold text-slate-900">All Retainers</h1>
             <ChevronDown size={18} className="text-[#1e61f0] mt-1" />
          </div>
          ...
          <table className="w-full text-left border-collapse">
              <thead className="bg-slate-50 text-[11px] font-bold text-slate-500 uppercase tracking-widest border-b border-slate-200">
                  <tr>
                      ...
                      <th className="px-6 py-4">Retainer Invoice Number</th>
                      <th className="px-6 py-4">Reference#</th>
                      <th className="px-6 py-4">Customer Name</th>
                      <th className="px-6 py-4">Project/Estimate</th>
                      <th className="px-6 py-4">Status</th>
                      <th className="px-6 py-4">Draw Status</th>
                      <th className="px-6 py-4 text-right pr-10">Amount</th>
                  </tr>
              </thead>
                    ...
                    filtered.map((r) => {
                        return (
                            ...
                            >
                                ...
                                <td className="px-6 py-4 text-[14px] font-medium text-[#1e61f0] group-hover:underline italic">
                                    {r.invoiceNumber}
                                </td>
                                <td className="px-6 py-4 text-[13px] text-slate-400 italic">
                                    {r.referenceNumber || '--'}
                                </td>
                                <td className="px-6 py-4 text-[14px] text-slate-900 font-medium uppercase">
                                    {r.CustomerLedger?.name || r.customerName}
                                </td>
                                <td className="px-6 py-4 text-[13px] text-slate-400 italic uppercase">
                                    {r.project || '--'}
                                </td>
                                <td className="px-6 py-4">
                                    <span className="text-[11px] font-bold text-slate-500 uppercase tracking-widest">{r.status}</span>
                                </td>
                                <td className="px-6 py-4">
                                    <span className="text-[11px] font-bold text-orange-600 uppercase tracking-widest">Await Payment</span>
                                </td>
                                <td className="px-6 py-4 text-right pr-10">
                                    <div className="flex items-center justify-end gap-6 font-medium text-[14px]">
                                        <span>₹ {parseFloat(r.totalAmount || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                                        <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <button onClick={(e) => { e.stopPropagation(); navigate(`/retainer-invoices/edit/${r.id}`); }} className="p-1.5 hover:text-blue-600"><Edit2 size={14} /></button>
                                            <button onClick={(e) => { e.stopPropagation(); setDeleteId(r.id); setIsDeleteModalOpen(true); }} className="p-1.5 hover:text-red-600"><Trash2 size={14} /></button>
                                        </div>
                                    </div>
                                </td>
                            </tr>
