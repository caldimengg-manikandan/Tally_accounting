...
async function run() {
  ...
  const query = `SELECT column_name FROM information_schema.columns WHERE table_name = 'UserCompanies';`;
  try {
    const res = await client.query(query);
    console.log("Executed successfully:", query);
    console.log("UserCompanies Rows:", res.rows.map(r => r.column_name).join(", "));
  } catch (e) {
